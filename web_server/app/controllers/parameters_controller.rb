class ParametersController < ApplicationController
  ...
  def index
    if params[:search]
      @parameters = Parameter.search(params[:search])
    else
      @parameters = Parameter.limit(100)
    end
  end
  ...
  def create
    @parameter = create_or_update(:PARAMETER_ID)
  ...
  private
    ...
    def parameter_params
      params.require(:parameter).permit(:PARAMETER_ID, :NAME, :DESCRIPTION, :EXPLAIN, :APPLY_STAT)
    end
end
