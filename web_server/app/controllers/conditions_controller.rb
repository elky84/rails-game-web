class ConditionsController < ApplicationController
  ...
  def index
    if params[:search]
      @conditions = Condition.search(params[:search])
    else
      @conditions = Condition.limit(100)
    end
  end
  ...
  def create
    @condition = create_or_update(:CONDITION_ID)
  ...
  private
    ...
    def condition_params
      params.require(:condition).permit(:CONDITION_ID, :CONDITION_EFFECT, :CONDITION_INFO, :PERCENT, :CONDITION_APPLY_TYPE)
    end
end
